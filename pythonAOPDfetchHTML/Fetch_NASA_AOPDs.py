...
def generate_urls(start_date_str, num_days):
    earliest_image_date = datetime.datetime.strptime(start_date_str, "%Y%m%d")
    urls = []                                                                   ## Initialize an empty list to store URLs 
    for i in range(num_days):                                                   ## Loop through the number of days
        # Calculate the previous date
        previous_date = earliest_image_date - datetime.timedelta(days=i)             ## Subtract days from the start date
...
# Generate URLs
urls = generate_urls(start_date_str, num_days)

# Fetch and save images
for url in urls:
    try:
        image_url, image_title = fetch_AOPD_with_URL(url)
        save_image("C:\\Users\\Public\\Pictures\\NASA_APOD_Images", image_url, image_title)
